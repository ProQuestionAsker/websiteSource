---
title: You Know Karen
authors: 
  - Jan Diehm
  - Sara Stoudt
  - Amber Thomas
date: '2020-06-20'
slug: karen
categories:
  - project
tags:
  - d3.js
  - data viz
  - The Pudding
buttons:
  - link: 'https://pudding.cool/2020/06/karen/'
    text: Interactive Article
description: 'But do you know the names that could be “other Karens” or “future Karens?” We found them using data.'
image: ''
imageAlt: ''
imageCredit: ''
imageCreditLink: ''
type: ''
---

## The Premise

“Karen” is having a moment — and that’s not a good thing. You’ve seen her at Red Lobster, stuck in traffic with Kidz Bop, racial profiling a Filipino man chalking “Black Lives Matter” outside his home and Black women at their apartment complex pool, and with her dog off leash endangering a Black man’s life in Central Park.

But how did the name Karen become cultural flashpoint “Karen” — an entitled middle-aged White woman who needs to speak to the manager? And what other names can be used in the same way as "Karen"? We used naming data from the Social Security Administration to find out. 

## My Contributions

* Some UX design
* Front-end development (HTML, CSS, and D3)

## Collaborators

This story was in collaboration with The Pudding's [Jan Diehm](https://twitter.com/jadiehm) and freelance contributor [Sara Stoudt](https://twitter.com/sastoudt). The idea was originally Jan's and she handled the bulk of the writing and design. Sara analyzed the data and worked with us to make sure we were displaying it correctly. 

## Screenshots
```{r echo = FALSE, out.width = "640", fig.align = "center"}
knitr::include_graphics("karen_video.gif")
```

```{r echo = FALSE, out.width = "100%"}
knitr::include_graphics("karen_1.png")
knitr::include_graphics("karen_2.png")
knitr::include_graphics("karen_3.png")
knitr::include_graphics("karen_4.png")
```